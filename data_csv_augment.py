# data augmentation tool for binary tabular datasets
# reads csv input and generates augmented parquet output
# uses controlled bit flipping for augmentation

from __future__ import annotations

from pathlib import Path
import argparse
import numpy as np
import pandas as pd


# get current script directory
script_dir = Path(__file__).resolve().parent

# default input and output files
input_file = "data.csv"
output_file = "data_augmented.parquet"

# column names used in processing
target_column = "Result"
source_row_id_column = "source_row_id"
is_augmented_column = "is_augmented"
augmentation_round_column = "augmentation_round"

# augmentation settings
augment_copies = 2
flip_probability = 0.02
random_seed = 42


def get_input_path(filename: str) -> Path:
    # load input file from current working directory
    # stop execution if the input file is missing
    file_path = script_dir / filename
    if not file_path.exists():
        raise FileNotFoundError(
            f"file not found: '{filename}'. place the file in the same directory as this software: {script_dir}"
        )
    return file_path


def get_output_path(filename: str) -> Path:
    # return path for saving output file
    return script_dir / filename


def load_csv_file(filename: str) -> pd.DataFrame:
    # read csv file into dataframe
    file_path = get_input_path(filename)
    return pd.read_csv(file_path)


def ensure_source_row_id(df: pd.DataFrame) -> pd.DataFrame:
    # ensure each row has a unique id for tracking
    df = df.copy()
    if source_row_id_column not in df.columns:
        df[source_row_id_column] = np.arange(len(df))
    return df


def prepare_original_rows(df: pd.DataFrame) -> pd.DataFrame:
    # mark original rows before augmentation
    df = df.copy()
    df[is_augmented_column] = 0
    df[augmentation_round_column] = 0
    return df


def get_binary_feature_columns(df: pd.DataFrame, target_col: str) -> list[str]:
    # identify binary feature columns (0/1 values only)
    excluded_columns = {
        target_col,
        source_row_id_column,
        is_augmented_column,
        augmentation_round_column,
    }

    # candidate columns excluding metadata
    candidate_columns = [col for col in df.columns if col not in excluded_columns]
    binary_columns = []

    # check each column contains only 0 and 1
    for column in candidate_columns:
        unique_values = set(df[column].dropna().unique().tolist())
        if unique_values.issubset({0, 1}):
            binary_columns.append(column)

    return binary_columns


def augment_row(
    row: pd.Series,
    binary_columns: list[str],
    rng: np.random.Generator,
    round_number: int,
) -> pd.Series:
    # create one augmented row by flipping bits randomly
    new_row = row.copy()

    # flip selected binary features with small probability
    for column in binary_columns:
        if rng.random() < flip_probability:
            new_row[column] = 1 - int(row[column])

    # mark as augmented row
    new_row[is_augmented_column] = 1
    new_row[augmentation_round_column] = round_number
    return new_row


def generate_augmented_data(
    df: pd.DataFrame,
    copies: int,
    target_col: str,
) -> pd.DataFrame:
    # generate augmented dataset for all rows
    binary_columns = get_binary_feature_columns(df, target_col)

    # ensure valid columns exist
    if not binary_columns:
        raise ValueError("no binary feature columns found to augment")

    rng = np.random.default_rng(random_seed)
    augmented_rows = []

    # create multiple augmented copies for each row
    for _, row in df.iterrows():
        for round_number in range(1, copies + 1):
            augmented_rows.append(
                augment_row(
                    row=row,
                    binary_columns=binary_columns,
                    rng=rng,
                    round_number=round_number,
                )
            )

    # return empty dataframe if nothing generated
    if not augmented_rows:
        return pd.DataFrame(columns=df.columns)

    return pd.DataFrame(augmented_rows, columns=df.columns)


def save_parquet_file(df: pd.DataFrame, filename: str) -> None:
    # save final dataframe as parquet file
    output_path = get_output_path(filename)
    df.to_parquet(output_path, index=False)
    print(f"saved: {output_path.name}")


def augment_csv_file(
    input_filename: str = input_file,
    output_filename: str = output_file,
    copies: int = augment_copies,
    target_col: str = target_column,
) -> pd.DataFrame:
    # main pipeline for csv augmentation

    # load and prepare data
    df = load_csv_file(input_filename)
    df = ensure_source_row_id(df)
    df = prepare_original_rows(df)

    # generate augmented data
    augmented_df = generate_augmented_data(df, copies, target_col)

    # combine original and augmented rows
    final_df = pd.concat([df, augmented_df], ignore_index=True)

    save_parquet_file(final_df, output_filename)
    return final_df


def parse_arguments():
    # read command line switches from the user
    parser = argparse.ArgumentParser(
        description="csv augmentation software for binary tabular datasets"
    )

    parser.add_argument(
        "--input",
        type=str,
        required=True,
        help="input csv file name",
    )

    parser.add_argument(
        "--output",
        type=str,
        required=True,
        help="output parquet file name",
    )

    parser.add_argument(
        "--target",
        type=str,
        default=target_column,
        help="target column name",
    )

    parser.add_argument(
        "--copies",
        type=int,
        default=augment_copies,
        help="number of augmented copies to create per row",
    )

    return parser.parse_args()


def main() -> None:
    # starting point of program
    args = parse_arguments()

    print("starting android permission dataset augmentation")
    print(f"software directory: {script_dir}")
    print(f"input file: {args.input}")
    print(f"output file: {args.output}")
    print(f"target column: {args.target}")
    print(f"copies: {args.copies}")
    print(f"flip probability: {flip_probability}")

    final_df = augment_csv_file(
        input_filename=args.input,
        output_filename=args.output,
        copies=args.copies,
        target_col=args.target,
    )

    # summary of results
    original_rows = (final_df[is_augmented_column] == 0).sum()
    augmented_rows = (final_df[is_augmented_column] == 1).sum()

    print(f"original rows: {original_rows}")
    print(f"augmented rows: {augmented_rows}")
    print(f"final rows: {len(final_df)}")
    print("process completed successfully")


if __name__ == "__main__":
    main()