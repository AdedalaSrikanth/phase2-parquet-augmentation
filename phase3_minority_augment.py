...
def get_minority_class_value(df: pd.DataFrame) -> int | float | str:
    class_counts = df[target_column].value_counts()
    return class_counts.idxmin()


def get_majority_class_count(df: pd.DataFrame) -> int:
    class_counts = df[target_column].value_counts()
    return int(class_counts.max())


def generate_minority_augmented_data(df: pd.DataFrame, mode: str) -> pd.DataFrame:
    ...
    minority_value = get_minority_class_value(df)
    majority_count = get_majority_class_count(df)

    minority_rows = df[df[target_column] == minority_value].copy()
    minority_count = len(minority_rows)

    if minority_count == 0:
        raise ValueError("minority class has no rows")
    ...
    copies_per_row = math.ceil(needed_rows / minority_count)

    augmented_rows = []

    for _, row in minority_rows.iterrows():
        for round_number in range(1, copies_per_row + 1):
    ...
    augmented_df = augmented_df.iloc[:needed_rows].copy()
...
def main() -> None:
    print("starting minority only augmentation")
